...
tf.app.flags.DEFINE_string(
    'dataset_split_name', 'training', 'The name of the train/test split.')
...
tf.app.flags.DEFINE_integer(
    'train_image_size', 220, 'Train image size')
...
def main(_):
  ...
  with tf.Graph().as_default():
    ...
    print ("\nNum of classes detected:"+str(num_classes))
    ...
    if dataset_type=='rgbd':
      ...
      image, label, depth = image_preprocessing_fn(image, train_image_size, train_image_size,
      ...
      tf.summary.image('image/',images,max_outputs=FLAGS.batch_size)
      tf.summary.image('label/',tf.cast(labels, tf.uint8),max_outputs=FLAGS.batch_size)
      tf.summary.image('depth/',depths,max_outputs=FLAGS.batch_size)

      labels = slim.one_hot_encoding(labels, num_classes)

      batch_queue = slim.prefetch_queue.prefetch_queue([images, labels, depths], capacity=2)
      images, labels, depths = batch_queue.dequeue()
      logits, end_points = network_fn(images, depth_inputs=depths)
      ...
      image, label = image_preprocessing_fn(image, train_image_size, train_image_size,resize_side_max=FLAGS.crop_larger_dim, resize_side_min=FLAGS.crop_smaller_dim,
                                            label=label)
    ...
    tf.summary.image('segmented_outputs/',tf.cast(tf.expand_dims(predictions,-1),tf.uint8),max_outputs=FLAGS.batch_size)
    #TODO
    ...
    # Gather update_ops from the first clone. These contain, for example,s
    # the updates for the batch_norm variables created by network_fn.
    update_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS)

    #summaries.add(tf.summary.scalar('mIoU',tf.cast(slim.metrics.streaming_mean_iou(predictions, orig_labels, num_classes,weights=mask)[0],tf.float64)))
    #summaries.add(tf.summary.scalar('Pixel Accuracy',slim.metrics.streaming_accuracy(predictions, orig_labels,weights=mask)[0]))
