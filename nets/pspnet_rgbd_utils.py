...
@slim.add_arg_scope
def pyramid_pooling_module(inputs, input_depth, levels, outputs_collections=None):
  with tf.variable_scope('pyramid_pool_module', [inputs, input_depth]) as sc:
    level_maps = [inputs]
    for level in reversed(levels):
      with tf.variable_scope(level.scope, 'level', [inputs, input_depth]) as sc:
        level_size, level_depth = level.args
        level_map = level.fn(input_depth, level_size, level_depth)
